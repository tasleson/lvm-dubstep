...
class Lvmetad(object):
    ...
    IDENT_CH = string.ascii_letters + string.digits + '_.-'
    WHITESPACE_CH = ' \t\n\r'
    NUMBER_CH = string.digits + '.'
    END = '''\n##\n'''
    ...
    @staticmethod
    def _ws(data, i):
        while data[i] in Lvmetad.WHITESPACE_CH:
            i += 1
        return i
    ...
    def _get_identifier(data, i):
        st = i
        while data[i] in Lvmetad.IDENT_CH:
            i += 1
        return data[st:i], i
    ...
    def _get_number(data, i):
        ...
        while data[i] in Lvmetad.NUMBER_CH:
            i += 1
    ...
    def _get_array(data, i):
        ...
        while True:
            ...
            if ch in Lvmetad.WHITESPACE_CH:
                i = Lvmetad._ws(data, i)
            elif ch == '#':
                i = Lvmetad._comment(data, i)
            else:
                # We should only be processing, numbers or strings
                if ch == '"':
                    value, i = Lvmetad._get_string(data, i)
                    rc.append(value)
                elif ch in Lvmetad.NUMBER_CH:
                    value, i = Lvmetad._get_number(data, i)
                    rc.append(value)
                elif ch == ',':
                    i += 1
                elif ch == ']':
        ...
        try:
            while True:
                ...
                if ch in Lvmetad.WHITESPACE_CH:
                    ...
                    if state == "U":
                        if ch in Lvmetad.IDENT_CH:
                            (identifier, i) = Lvmetad._get_identifier(data, i)
                            state = "A"
                        elif nested and ch == '}':
                            i += 1
                            break
                        ...
                        if ch == '=':
                            ...
                            i += 1
                            rc[identifier], i = \
                                Lvmetad._get_object(data, i, True)
                            state = 'U'
                        else:
                            raise Exception(
                                "Unexpected character %s, pos= %d, "
                        ...
                        if ch == '[':
                            rc[identifier], i = Lvmetad._get_array(data, i)
                            state = 'U'
                        elif ch == '"':
                            rc[identifier], i = Lvmetad._get_string(data, i)
                            state = 'U'
                        elif ch in string.digits:
                            # Does the format support negative numbers, doesn't
                            # appear too based on `man 5 lvm.conf`
                            rc[identifier], i = Lvmetad._get_number(data, i)
                            state = 'U'
                        else:
                            raise Exception(
                                "Unexpected character %s, pos= %d, "
    ...
    @staticmethod
    def parse(data):
        return Lvmetad._get_object(data, 0)[0]
